const Attachments = require('../../models/placement/placement_attachments.model');
const fs = require('fs');
...
exports.create = (req, res) => {
    ...
    const attachments = new Attachments({
        ...
        filePath: "./Uploads/placement/" + req.body.fileName,
    });
...
exports.updateById = (req, res) => {
    ...
    if (req.body.file != null) {
        let buff = new Buffer.from(req.body.file, 'base64');
        let filePath = "../Uploads/placement/" + req.body.fileName
    ...
    Attachments.findOneAndUpdate({ _id: req.params._id }, {
        ...
        .then(data => {
            if (!data) {
                return res.status(404).send({
                    message: "Attachment not found with id " + req.params._id
                });
            }
            res.send(data);
        }).catch(err => {
            ...
            return res.status(500).send({
                message: "Error updating Attachment with id " + req.params._id
            });
        });
};
...
exports.deleteById = (req, res) => {
    Attachments.findOneAndRemove(req.params._id)
        .then(data => {
            ...
            return res.status(500).send({
                message: "Could not delete Attachment with id " + req.params._id
            });
        });
};
